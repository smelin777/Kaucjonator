...
export default function Kaucjonator() {
    ...
    (async () => {
      try {
        ...
        try {
          ...
          /* brak ustawieĹ - uĹźyj domyĹlnych */
        }
  ...
  const saveSettings = useCallback(async (newRate, newGoal) => {
    try {
      ...
      console.error("BĹÄd zapisu ustawieĹ", e);
    }
  }, []);
  ...
  const generateShareImage = () =>
    new Promise((resolve) => {
      ...
      ctx.fillText("DZISIAJ ZEBRAĹEM", canvas.width / 2, cardY + 44);
      ...
      ctx.fillText(`${money(count)} zĹ`, canvas.width / 2, cardY + 168);
    ...
    try {
      ...
      if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: "Kaucjonator",
          text: `Dzisiaj zebraĹem ${count} butelek i ${money(count)} zĹ kaucji! đž`,
        });
        setShareMsg("UdostÄpniono!");
      } else {
      ...
      setShareMsg("Nie udaĹo siÄ udostÄpniÄ.");
    } finally {
      setSharing(false);
